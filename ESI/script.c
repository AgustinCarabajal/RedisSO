...
void envio_operaciones(char* operacion,char* clave, int socket){
	...
	char* buffer = serializar(operacion,clave);
	int estado = send(socket, buffer, 7 + strlen(clave), 0);
	...
	eliminarPaquete(&buffer);
}

void envio_operacion_set(char* operacion,char* clave, char* valor, int socket){

	char* buffer = serializar_set(operacion, clave, valor);
	int estado = send(socket,buffer , 11 + strlen(clave) + strlen(valor), 0);
